    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="ghost"
          size="sm"
          className="text-foreground hover:text-primary flex items-center space-x-2 px-3 md:px-3"
        >
          ...
          <div className="space-y-1 max-h-64 overflow-y-auto">
            {Object.entries(languages).map(([key, language]) => (
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2.5 text-sm rounded-md hover:bg-accent/50 transition-colors ${
