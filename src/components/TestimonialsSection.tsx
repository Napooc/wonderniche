  ...
  return <section className="py-20 px-4 relative overflow-hidden">
      <div className="container mx-auto max-w-6xl">
              ...
              <div className="text-center space-y-8">
                ...
                <div className="flex flex-col items-center space-y-4">
                  <Avatar className="w-20 h-20 ring-4 ring-primary/30 ring-offset-4 ring-offset-background">
                    <AvatarImage src={testimonials[activeTestimonial].image} alt={testimonials[activeTestimonial].name} />
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-16">
          {testimonials.filter((_, index) => index !== activeTestimonial).slice(0, 3).map((testimonial, index) => <Card key={testimonial.id} className={`glass-card p-6 cursor-pointer transition-all duration-300 hover:scale-105 reveal-up stagger-${index + 2}`} onClick={() => setActiveTestimonial(testimonials.findIndex(t => t.id === testimonial.id))}>
      ...
      <div className="absolute bottom-10 right-10 w-32 h-32 rounded-full bg-accent/10 blur-2xl animate-float" style={{
      animationDelay: '-3s'
    }} />
    </section>;
};
