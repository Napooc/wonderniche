...
import { Search, Filter, Grid, List, MapPin, Luggage, Smartphone, Coffee } from 'lucide-react';
import { useTranslation } from '@/contexts/TranslationContext';
import { generateTravelSEO } from '@/utils/seo';
...
const Travel = () => {
  ...
  const seoData = generateTravelSEO(currentLanguage);
  ...
  const fetchProducts = async () => {
    try {
      const {
        data: categoriesData
      } = await supabase.from('categories').select('id').eq('slug', 'travel').single();
      if (categoriesData) {
        const {
          data: productsData
  ...
  return <div className="min-h-screen">
      ...
      {/* Hero Section */}
      <CategoryHero title={t('travel.title')} subtitle={t('travel.subtitle')} images={[travelHero1]} ctaText={t('travel.exploreGear')} onCtaClick={() => document.getElementById('products')?.scrollIntoView({
      behavior: 'smooth'
    })} />
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center space-y-4 reveal-up stagger-1">
              <div className="w-16 h-16 mx-auto rounded-full bg-gradient-to-r from-blue-500 to-cyan-500 p-4">
                <Luggage className="w-full h-full text-white" />
              </div>
              <h3 className="text-xl font-semibold">{t('travel.luggageEssentials')}</h3>
              <p className="text-muted-foreground">
                {t('travel.luggageDesc')}
              </p>
            </div>
            ...
            <div className="text-center space-y-4 reveal-up stagger-2">
              ...
              <h3 className="text-xl font-semibold">{t('travel.techGear')}</h3>
              <p className="text-muted-foreground">
                {t('travel.techGearDesc')}
              </p>
            </div>
            ...
            <div className="text-center space-y-4 reveal-up stagger-3">
              ...
              <h3 className="text-xl font-semibold">{t('travel.comfortItems')}</h3>
              <p className="text-muted-foreground">
                {t('travel.comfortDesc')}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="mb-12 space-y-6 reveal-up">
            <div className="text-center">
              <h2 className="text-4xl md:text-5xl font-bold gradient-text mb-6">
                {t('travel.premiumGear')}
              </h2>
              <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
                {t('travel.everythingNeeded')}
              </p>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
              <div className="relative flex-1 max-w-md">
                ...
                <input type="text" placeholder={t('travel.searchPlaceholder')} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-input rounded-full text-foreground placeholder:text-muted-foreground border border-border focus:outline-none focus:ring-2 focus:ring-primary" />
              </div>
          ...
          {loading ? <div className="text-center py-12">
              ...
              {filteredProducts.map((product, index) => <Link key={product.id} to={`/product/${product.id}`} className={`reveal-up stagger-${index % 4 + 1} block transition-transform hover:scale-[1.02]`}>
                  <ProductCard id={product.id} title={product.name} description={product.short_description || product.description} rating={product.rating || 4.5} reviews={product.reviews_count || 0} image={product.image_url || 'https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=600&h=600&fit=crop'} category="Travel" affiliateUrl={product.affiliate_url} isNew={product.is_featured} />
                </Link>)}
            </div>}
